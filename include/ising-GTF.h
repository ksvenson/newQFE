...
class QfeIsing {
 public:
  ...
  // double MeanSpinGTF();
  int SumSpinGTF() ;
  void SumLinkGTF(std::vector<int> &sum, int num_dirs=0) ;
  void HotStart();
  void ColdStart();
  double Metropolis();
  int WolffUpdate();
...
//double QfeIsing::MeanSpinGTF() {
//  double m = 0.0;
//  int count = 0 ;
//  for (int s = 0; s < lattice->n_sites; s++) {
//    if (lattice->sites[s].nn >= 1) {
//      m += spin[s] * lattice->sites[s].wt;
//      count++ ;
//    }
//  }
...
/// @brief Sum all the spins, excluding sites that have no links to neighbors.
/// @return The spin sum (i.e. magnetization)
int QfeIsing::SumSpinGTF() {
  int m = 0 ;
  for (int s = 0; s < lattice->n_sites; s++) {
    if (lattice->sites[s].nn >= 1) {
      m += spin[s] > 0. ? 1 : -1 ;
...
/// @brief Sum (s_i * s_j) across all links pointing in the same direction
/// @param vector of sums
void QfeIsing::SumLinkGTF(std::vector<int> &sum, int num_dirs) {

  sum.resize(num_dirs) ;
  sum.assign(num_dirs, 0) ;
  
  for (int s = 0; s < lattice->n_sites; s++) {
    int num_links = lattice->sites[s].nn ;

    // skip site s if (num_links < 1)
    if (num_links >= 1) {

      // Only sum links that are in positive directions from 1 to num_dirs
      for (int n = 0;  n < num_links; n++) {
        int dir = lattice->sites[s].linkdirs[n] ;
        if (dir > 0 && dir <= num_links) {
          sum[dir - 1]
            += (spin[s] > 0. ? 1 : -1) * (spin[lattice->sites[s].neighbors[n]] > 0. ? 1 : -1 ) ;
        }
      }
    }
  }
...
int QfeIsing::WolffUpdate() {
  // GTF: don't cluster sites with no links to neighbors
