...
// msq = -1.27 [1]. On a 64^2 lattice it takes about 4 wolff cluster sweeps to
// get a net cluster size roughly equal to the entire lattice. After the cluster
// update, we perform an metropolis sweep and then a overrelaxation sweep. We
// take measurements every 20 iterations, which leads to reasonable
// autocorrelation times for the 2nd and 4th magnetic moments. With these
// parameters, we expect the 4th order binder cumulant to be near its critical
// value of 0.8. we also expect to see a peak in the magnetic susceptibility,
// which can be seen by varying msq. We can also check that the overralation
// demon is close to 1.
