...
struct QfeSite {
  ...
  int linkdirs[MAX_SITE_NEIGHBORS];   // direction of links (default 0 means no assigned direction)
...
class QfeLattice {
 public:
  ...
  // BEGIN GTF
  void InitCubic(std::vector<int> &L, std::vector<double> &sc, std::vector<double> &fcc,
                 std::vector<double> &bcc, bool fullTransInv = false) ;
  //  END  GTF
  ...
  int AddLinkGTF(int a, int b, double wt, int dir=0);
...
// BEGIN GTF
void
QfeLattice::InitCubic(std::vector<int> &L, std::vector<double> &sc, std::vector<double> &fcc,
               std::vector<double> &bcc, bool fullTransInv)
{
  //Check the dimensions of vectors
  if (L.size() != 3 || sc.size() != 3 || fcc.size() != 6 || bcc.size() != 4) {
    std::cerr << "ERROR: L " << L.size() << " sc " << sc.size() << " fcc "
      << fcc.size() << " bcc " << bcc.size() << std::endl ;
    exit(-1) ;
  }
  ...
  for (int s = 0; s < n_sites; s++) {
    ...
    // int xm = (x - 1 + L[0]) % L[0] ; // not needed
    int ym = (y - 1 + L[1]) % L[1] ;
    int zm = (z - 1 + L[2]) % L[2] ;

    // Add "forward" simple cubic (SC) links with non-zero weight.
    if (sc[0] != 0.) {
      AddLinkGTF(s, xp + L[0]*(y + L[1]*z), sc[0], 1) ;
    }
    if (sc[1] != 0.) {
      AddLinkGTF(s, x + L[0]*(yp + L[1]*z), sc[1], 2) ;
    }
    if (sc[2] != 0.) {
      AddLinkGTF(s, x + L[0]*(y + L[1]*zp), sc[2], 3) ;
    }

    // Add "forward" fcc links to even sites with non-zero weight.
    //   or add to all sites if (fullTransInv == true)
    if (((x+y+z) % 2 == 0) || fullTransInv == true) {
      if (fcc[0] != 0.) {
        AddLinkGTF(s, xp + L[0]*(yp + L[1]*z), fcc[0], 4) ;
      }
      if (fcc[1] != 0.) {
        AddLinkGTF(s, xp + L[0]*(y + L[1]*zp), fcc[1], 5) ;
      }
      if (fcc[2] != 0.) {
        AddLinkGTF(s, x + L[0]*(yp + L[1]*zp), fcc[2], 6) ;
      }
      if (fcc[3] != 0.) {
        AddLinkGTF(s, xp + L[0]*(ym + L[1]*z), fcc[3], 7) ;
      }
      if (fcc[4] != 0.) {
        AddLinkGTF(s, xp + L[0]*(y + L[1]*zm), fcc[4], 8) ;
      }
      if (fcc[5] != 0.) {
        AddLinkGTF(s, x + L[0]*(yp + L[1]*zm), fcc[5], 9) ;
      }
    }

    // Add "forward" bcc links to bcc sites with non-zero weight.
    //   or add to all sites if (fullTransInv == true)
    if (((x % 2 == y % 2) && (y % 2 == z % 2)) || fullTransInv == true) {
      if (bcc[0] != 0.) {
        AddLinkGTF(s, xp + L[0]*(yp + L[1]*zp), bcc[0], 10) ;
      }
      if (bcc[1] != 0.) {
        AddLinkGTF(s, xp + L[0]*(yp + L[1]*zm), bcc[1], 11) ;
      }
      if (bcc[2] != 0.) {
        AddLinkGTF(s, xp + L[0]*(ym + L[1]*zp), bcc[2], 12) ;
      }
      if (bcc[3] != 0.) {
        AddLinkGTF(s, xp + L[0]*(ym + L[1]*zm), bcc[3], 13) ;
      }
    }
  } // end for (int s = 0; s < n_sites; s++)
}
//  END  GTF
...
/**
 * @brief Adds a link from site @p a to @p b with weight @p wt, optionally with a label @p
 * indicating the direction from the first site to the second. Returns the
 * link index.
 */

int QfeLattice::AddLinkGTF(int a, int b, double wt, int dir) {
  int l = links.size();  // link index
